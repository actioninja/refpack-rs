...
pub enum Error {
    ...
    BadOffset,
    /// Error indicating that the requested copy offset was larger than the
    /// length of the buffer to copy from this means that the copy function
    /// would try to copy from before the start of the buffer which is an
    /// illegal operation
    #[error("Offset went past start of buffer: buffer length `{0}`, offset `{1}`")]
    NegativePosition(usize, usize),
    /// Indicates that the decompressed file would be larger than the indicated
    /// size in the header this is important to prevent accidental massive
    /// memory usage
