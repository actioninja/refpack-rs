...
impl Command {
    ...
    fn new(offset: usize, length: usize, literal: usize) -> Self {
        if literal > 3 {
    ...
    fn new_literal(length: usize) -> Self {
        if length > 112 {
    ...
    fn new_stop(literal_length: usize) -> Self {
        if literal_length > 3 {
    ...
    fn num_of_literal(&self) -> Option<usize> {
        match self {
            Command::Short {
                literal,
                ..
...
struct ControlIterator<'a, R: Read + Seek> {
    reader: &'a mut R,
    reached_stop: bool,
}
...
pub fn compress<R: Read + Seek, W: Write>(reader: &mut R, writer: &mut W) {
