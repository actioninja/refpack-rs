...
use std::ops::Range;
use crate::data::compression::match_length::{byte_offset_matches, match_length, match_length_except, match_length_or};
...
impl<'a, const N: usize> PrefixSearcher<'a, N> {
    fn search_break<F: FnMut(usize, usize)>(
        ...
        'outer: loop {
            ...
            for n in 0..N {
                let n_match_length = prev[from].prev[n].match_length;
                if n_match_length == 0 || n_match_length > prev_matched_len {
                    ...
                    let match_len = match_length(buffer,
                                                 pos,
                                                 match_pos as usize,
    ...
    fn search_from_offset<F: Fn(u32, u16) -> u16>(prev: &[HashChainLink<N>],
                                                  pos: usize,
                                                  min_length: usize,
                                                  mut from: usize,
                                                  mut prev_matched_len: u16,
        ...
        'outer: loop {
            ...
            loop {
                ...
                if pl.match_length == 0 {
                    if level == 0 {
                        return None;
                    } else {
                        level -= 1;
                    }
                } else if prev_matched_len < pl.match_length {
                    if level == 0 {
                        // from = pl.position as usize;
                        from = p.short_skip_pos as usize;
                        continue 'outer;
                    } else if prev_matched_len > p.prev[level - 1].skip_match_length {
                        from = pl.position as usize;
                        continue 'outer;
                    } else {
                        level -= 1;
                    ...
                    if match_len as usize == pl.match_length as usize {
                        if pl.bad_position == u32::MAX || (pl.bad_position as usize) < long_offset_limit {
                            return None;
                        }
                ...
                } else if prev_matched_len <= pl.skip_match_length {
                    prev_matched_len = pl.match_length;
                    from = pl.position as usize;
                    continue 'outer;
                } else { // prev_matched_len > pl.skip_match_length
                    if level == N - 1 {
                        prev_matched_len = pl.match_length;
                        from = pl.position as usize;
                        continue 'outer;
                    } else if p.prev[level + 1].match_length == 0 {
                        return None;
                    } else {
                        level += 1;
                    }
    ...
    pub fn build(buffer: &'a [u8]) -> Self {
        ...
        let mut prev = buffer.windows(3)
            .enumerate()
            .map(|(i, window)| {
                let p = prefix(window);

                let prev_pos = head.get(&p).cloned();
                let mut short_skip_pos = u32::MAX;
                let new = prev_pos.map(|(prev_pos, prev_len, prev_short_skip)| {
                ...
                head.insert(p, (i, new.match_length, short_skip_pos));
        ...
        for i in 0..prev.len() {
            if prev[i].prev[0].match_length > 0 {
                for o in 0..N {
                    let n = o + 1;
                    ...
                    prev[i].prev[o].skip_match_length = slen as u16;

                    if slen < min(LONG_LENGTH_MAX as usize, buffer.len() - spos - 1) {
                        if n < N {
                            if let Some((fpos, flen))
                                = Self::search_from_offset(
                                &prev,
                                i,
                                po.match_length as usize,
                                po.position as usize,
                                po.match_length,
                                |pos, skip| {
                                    match_length_or(buffer,
                                                    i,
                                                    pos as usize,
                                                    po.position as usize,
                                                    po.match_length as usize,
                                ...
                                if byte_offset_matches(
                                    ...
                                    if flen > slen {
                                        ...
                                        prev[i].prev[n].match_length = flen as u16;
                                        continue;
                                    }
                                } else {
                                ...
                                if let Some((pos, len))
                                    ...
                                    |pos, skip| {
                                        match_length(buffer,
                                                     ...
                                                     pos as usize,
                                                     LONG_LENGTH_MAX as usize,
                                                     skip as usize) as u16
                                    }) {
                                    prev[i].prev[n].position = pos as u32;
                                    prev[i].prev[n].match_length = len as u16;
                                } else {
                                    break;
                                }
                            } else {
    ...
    pub fn search<F: FnMut(usize, Range<usize>)>(&mut self, pos: usize, mut found_fn: F) {
        ...
        for cur_match in &start_match.prev[..N - 1] {
            ...
            let match_len = cur_match.match_length as usize;
            ...
            Self::search_break(self.buffer,
                               &self.prev,
                               pos,
                               match_pos,
                               match_len as u16,
                               |pos, len| {
                                   if len <= cur_match.skip_match_length as usize {
        ...
        if start_match.prev[N - 1].match_length > 0 {
            let mut cur_match = Some((start_match.prev[N - 1].position as usize,
                                      start_match.prev[N - 1].match_length as usize));
